...
import { SonatypeOSSVulnFetcher } from '../../vulnerability-mgmt/SonatypeOSSVulnFetcher';

const my_map = (value, x1, y1, x2, y2) => ((value - x1) * (y2 - x2)) / (y1 - x1) + x2;

export default function NoSSRGraph(props) {
  const maxDepsCount = 262;
  const graphRef = useRef<ForceGraphMethods>();
  const f = new SonatypeOSSVulnFetcher();
  ...
  useEffect(() => {
    ...
    r.d3Force('y', forceY());
    r.d3Force('link')
      .distance((link) => (1 * (link.source.deps_count + link.target.deps_count * 2)) / maxDepsCount)
      // .distance((link) => 1000)
      .strength(1);
